#ifndef TABLE8_INCLUDED
#define TABLE8_INCLUDED
...
public:
   ...
   */
  Table8(const int width, const int height, const int default_value = 0);

  /*!
   * @brief Contructor
   * @param[in] tmp  Copy source.
   */
  Table8(const Table8& tmp);
   ...
   * Copy from source.
   */
  Table8& operator = (const Table8& tmp);
   ...
   * Compare source.
   */
  bool Equal(const Table8& tmp) const;

//protected:
  int8_t  width_;   //!< Map width.
  int8_t  height_;  //!< Map height.
  int16_t size_;
  int8_t* data_;    //!< Map data.
};

///////////////////////////////////////////////////////////////////////////////
inline bool operator == (const Table8& a, const Table8& b) {
  return a.Equal(b);
}
...
class hash<Table8> {
public:
  std::size_t operator()(const Table8& table) const {
    return table.Hash();
  }
};
